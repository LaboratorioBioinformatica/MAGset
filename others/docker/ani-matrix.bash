...
for i in "${!SEQS[@]}" ; do
  for j in "${!SEQS[@]}"; do
    ...
    echo -n " ${SEQS[i]} vs ${SEQS[j]}: "
    ANI=$(ani.rb -1 "${SEQS[i]}" -2 "${SEQS[j]}" -S "$OUT.db" -t "$THR" \
      --no-save-rbm --no-save-regions --auto --quiet)
    if [[ "$ANI" == "" ]]
    then
      echo "It is not possible get a reliable ANI result between the files ${SEQS[i]} and ${SEQS[j]}. Please use genomes of the same species."
    fi
